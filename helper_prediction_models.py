...
path_pred_res = "/Users/sumyea/Desktop/Study/Study/SS23/MLAPM/Code/Inter_case_aware_RTM-master/time-prediction-benchmark-master/"
path_preprocessed_data = "/Users/sumyea/Desktop/Study/Study/SS23/MLAPM/Code/Inter_case_aware_RTM-master/time-prediction-benchmark-master/experiments/logdata"
path_pred_data_save = "/Users/sumyea/Desktop/Study/Study/SS23/MLAPM/Code/Inter_case_aware_RTM-master/next_activity_pred/"
path_pred_activity_res = "/Users/sumyea/Desktop/Study/Study/SS23/MLAPM/Code/Inter_case_aware_RTM-master/next_activity_pred/Results_production/"
path_new_rtp_data = "/Users/sumyea/Desktop/Study/Study/SS23/MLAPM/Code/Inter_case_aware_RTM-master/time-prediction-benchmark-master/experiments/feature_enriched_data_new"
...
def train_starting_batch_dynamics_model(df,a1,a2,cols,file, segparams):
    ...
    X = filter_df[["starting cases", "ending cases", "pending cases", "LES", "time_since_last_event", "observations"
        ...
        , "batch_month", "batch_hour", "is_weekend", "weekday"
        ,  "Resource", "behaviour class", "size class"]] #month, duration
    y = filter_df["waiting_time"]
    numerical_features = ["starting cases", "ending cases", "pending cases", "LES", "time_since_last_event",
                          "observations",
                          "mean_IBCR", "std_IBCR", "min_delay", "max_delay"] #duration
    categorical_features = ["batch_day", "batch_month", "batch_hour", "is_weekend", "weekday",  "Resource",
                            "behaviour class", "size class"] #month
    for feature in categorical_features:
        X[feature] = X[feature].astype('category')
    X_train, X_test, y_train, y_test = split_data(X, y)
...
def train_starting_batch_dynamics_average_model(df,a1,a2,cols,file, segparams):
    ...
    X = data_pred[["starting cases", "ending cases", "pending cases", "LES", "time_since_last_event", "observations"
        , "mean_IBCR", "std_IBCR", "min_delay", "max_delay", "batch_day", "batch_month", "batch_hour", 
                   "is_weekend", "weekday"
        , "current_batch_observations", "avg_waiting_time_lag_1"]] #month, duration
    y = data_pred["waiting_time"]
    numerical_features = ["starting cases", "ending cases", "pending cases", "LES", "time_since_last_event",
                          "observations",
                          "mean_IBCR", "std_IBCR", "min_delay", "max_delay",
                          "current_batch_observations", "avg_waiting_time_lag_1"]#duration
...
def train_end_batch_dynamics_model(df,a1,a2,cols,file, segparams):
    ...
    filter_df["prev_batch_BM"] = pd.to_datetime(np.where(filter_df["prev_batch_BM"] == 0, pd.to_datetime("1990-01-01 00:00:00", utc=True), filter_df["prev_batch_BM"]), utc=True)
...
def get_segment_prediction(data_file,activity_indice,uncertain_segments,file):
    ...
    next_activity_preds = pd.read_csv(path_pred_activity_res + "/result_" + file + '.csv')
    next_activity_preds.rename(columns={"CaseID": "Case ID"})
    ...
    merge_df["next_activity_prediction"] = merge_df['Activity'].shift(-1)
    ...
    print('uncertain segment dict')
    print(uncertain_seg_dict)
    merge_df["Segment_Prediction"] = merge_df["merged_activity"].apply(lambda x: uncertain_seg_dict.get(x, 0))
    unique_segment = merge_df["Segment_Prediction"].unique()
    print("unique segments", unique_segment)
    print('merge df after segment pred')
...
def get_predicted_segment_data(df, a1, a2, cols):
    ...
    # filter for activities a1,a2
    #data["merged_activity"] = data.apply(lambda x: x["Activity"] + "_" + x["Activity_1"], axis=1)
    data["merged_activity"] = data['Activity'].astype(str) + '_' + data['Activity_1']
    ...
    #print('filter_df')
    #print(filter_df)
    filter_df = filter_df.rename(columns = {"Complete Timestamp": "ta", "Complete Timestamp_1": "tb"})
    #print('filter_df after rename')
...
def test_starting_batch_dynamics_model(df,a1,a2,cols,file,segClass, merge_df, train_ids):
    ...
    X = filter_df[["starting cases", "ending cases", "pending cases", "LES", "time_since_last_event", "observations"
        ...
        , "duration", "Resource", "behaviour class", "size class"]]
    y = filter_df["waiting_time"]
    numerical_features = ["starting cases", "ending cases", "pending cases", "LES", "time_since_last_event",
                          "observations",
                          "mean_IBCR", "std_IBCR", "min_delay", "max_delay", "duration"]
...
def test_end_batch_dynamics_model(df, a1, a2, cols, file, segClass, merge_df, train_ids):
    ...
    x = filter_df.groupby("tb").size().reset_index()[1:]
    batch_df = x["tb"].apply(lambda x: tmp_batch[tmp_batch["tb"] < x].tail(1).squeeze())
    batch_df = batch_df.rename(columns = {"tb": "prev_batch_BM"})
    batch_df["tb"] = x["tb"]
    filter_df = filter_df.merge(batch_df, on='tb', how='left', suffixes=('_1', '_2')).fillna(0)
    ...
    filter_df["prev_batch_BM"] = pd.to_datetime(
        np.where(filter_df["prev_batch_BM"] == 0, pd.to_datetime("1990-01-01 00:00:00", utc=True), filter_df["prev_batch_BM"]), utc=True)
    # filter_df["prev_batch_BM"] = pd.to_datetime(filter_df["prev_batch_BM"])
    filter_df["prev_batch_BM"] = np.where(filter_df["Case ID"].isin(obs_first_batch), filter_df["ta"],
                                          filter_df["prev_batch_BM"])
    filter_df["time_since_last_batch"] = (filter_df["ta"] - filter_df["prev_batch_BM"]).dt.total_seconds()
