...
path_pred_data_save = "../ProcessSequencePrediction/data"
path_pred_activity_res = "../ProcessSequencePrediction/code/output_files/results"
path_new_rtp_data = "/Users/sumyea/Desktop/Study/Study/SS23/MLAPM/Code/Inter_case_aware_RTM-master/time-prediction-benchmark-master/experiments/results/feature_enriched_data"
...
def get_threshold_prefix(df,variants_count):
    ...
    freq_variants = list(filter(lambda x: (len(x['variant'])<=threshold_prefix) & (x['count']>0),variants_count))
...
def get_relevant_variants(a1,a2,freq_variants):
    ...
    for trace in variants:
        trace_dict = {k: v for v, k in enumerate(trace)}
...
def get_relative_error_test(relevant_cases,p1,p2,file):
    # calculate relative error for relevant cases based on prediction results
    #read prediction result files to calculaute relative errors
    df_a1 = pd.read_csv(path_pred_res+"results/feature_enriched_log_results/results_test_"+file+"_"+str(p1)+".csv")
#     df_a1 = df_a1[df_a1["true_value"]!=0]
    df_a2 = pd.read_csv(path_pred_res+"results/feature_enriched_log_results/results_test_"+file+"_"+str(p2)+".csv")
#     df_a2 = df_a2[df_a2["true_value"]!=0]
    if(p1!=1):
        cases_p1 = relevant_cases["Case ID"].apply(lambda x: "%s_%s"%(x, p1))
    else:
...
def get_train_data_segment_analysis(threshold_prefix,file,df):
    ...
    for n in range(2,threshold_prefix):
        ...
        df_a1 = pd.read_csv(path_pred_res+"results/feature_enriched_log_results/results_train_"+file+"_"+str(p1)+".csv")
        df_a2 = pd.read_csv(path_pred_res+"results/feature_enriched_log_results/results_train_"+file+"_"+str(p2)+".csv")
...
#     ,bbox_to_anchor=(0.5, 1.05)
    legend = ax.legend(shadow=True, fancybox=True, title="Activity b", bbox_to_anchor=(0., 1.02, 1., .102),loc=3, mode="expand", fontsize=12, ncol=3)
#     ax.set_title("Magnitude of Relative Error Change for each Case Passing through a Segment",fontsize=18,weight='bold')
    
    ax.text(x=0.5, y=1.5, s='Magnitude of Difference between Relative Prediction Errors for each Case Passing through a Segment S=(a,b)', fontsize=16, weight='bold', ha='center', va='bottom', transform=ax.transAxes)
